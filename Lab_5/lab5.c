...
//created by Martin Alejo #75296665, and Jonathan Li #97939987
...
#define degreemode P0_0
#define radianmode P0_3
...
void main (void)  //using ch0 and ch2 for the sin signals 
{
   ...
   // Measure half period at ADC CH2 using timer 0
		...
		if(mode == 1){	
			...
			sprintf(secondrow,"3:%4.2frms %4.2fr",pVolt1rms,phase);
			if(phase<0.0){
				if(phase < -20.0*PI/180.0){
				phase = phase - 5.0*PI/180;
				sprintf(secondrow,"3:%4.2frms %4.2fr",pVolt1rms,phase);
				}
				else if(phase > -20*PI/180 && phase < 0.0){
					phase = phase - 3.0*PI/180;
					sprintf(secondrow,"3:%4.2frms %4.2fr",pVolt1rms,phase);
				}
			}
		}
